...
One can show that $\F_{k-1} \subset \F_k$ for $k=0, \dots, k_f$.
...
Note that $\alpha\in \F_0$, hence $T_0\subset \F_0$.

Take $k$ from $0, \dots, k_f$. 

Note that $(\M_{k-1}, j)\models T_k$. 
...
\begin{enumerate}
\item (AXM) $\gamma_{r_k}\in T_k \cup \mathfrak{A}$. Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\F_k}$. If $\gamma_{r_k}\in T_k$, then $(\M_k, j)\models\gamma_{r_k}$. If $\gamma_{r_0}\in \mathfrak{A}$, then $(\M_k, j)\models\gamma_{r_k}$ since $\gamma_{r_k}$ is an logical axiom. Note that the above argument works for $k = 0$ without any of induction assumptions. 
...
\item \label{gen} (GEN) there exists $\eta < r_k$ and $x\in \V$ such that
...
Then $(\M_k, j)\models \beta(x/\sk^m_{r_k})$, which is the same as $(\M_k,j)\models \gamma_{r_k}$.

\end{enumerate}
\item $\context(D, r_k) \not= \emptyset$.
Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\F_k}$.
By Definition \ref{proof-definition}, $\gamma_{r_k}\in \F_k$.
Since $T_k\subset\F_k$,
one can verify that $\context(D, r_k)$ is a proof of $\gamma_{r_k}$ with respect to $\mathfrak{A}\cup T_k$. Since $\M_k\models T_k$ and $|\context(D, r_k)| < n$, by induction assumption (\ref{induction-1}), we have $(\M_k, j)\models\gamma_{r_k}$.
...
But $\gamma_{r_{k_f}} = \beta$ where 
$\phi = \ulcorner \alpha\to\beta \urcorner \in \F(\L_r)$. Since $\M_{k_f}$ is an extension of $\M$, we have $(\M, j)\models\beta$ and then $(\M, j)\models\phi$.
