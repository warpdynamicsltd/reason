...
\begin{definition}
Let $\L$ be a signature of a language.
Let $(D, \L^+,\gamma)$ be a reference system and $T\subset \F(\L^+)$ and $\rho\in D$.
\begin{equation}
\refset_D(\rho, T) = \{\gamma_n\in D: n < \rho\}\cup T.
\end{equation}
\end{definition}
...
Let $\L$ be a signature of a language.
A reference system $(D, \L^+,\gamma)$ is a constrained-proof of $\psi\in\F(\L^+)$ with respect to $\mathcal{A}\subset \F(\L^+)$ with fixed variables $V\subset \V$ iff for $r = \root(D)$, $\gamma_r = \psi$
and for any $\rho\in \overline{D}$ the following conditions holds:
\begin{enumerate}
\item \label{shallow-part} if $\context(D, \rho) = \emptyset$, then $\gamma_\rho\in\F(\overline{\L}_\rho)$ and at least one of the following holds:
\begin{enumerate}
...
%\item (ISO) there exists $\eta < \rho$ and 
%$\gamma_\eta$ is isomporphic to $\gamma_\rho$,
\item (MOD) there exist $\phi_1, \phi_2\in \refset_D(\mathcal{A}, \rho)$, such that
\begin{equation}
\phi_1 = \ulcorner \phi_2 \to \gamma_\rho \urcorner,
\end{equation}
\item (GEN) there exists $\phi \in \refset_D(\mathcal{A}, \rho)$ and $x\in \V\setminus (V \cup \fixed_D(\rho))$ such that
\begin{equation}
\gamma_\rho = \ulcorner \forall x. \phi \urcorner,
\end{equation}

\item (SKO) there exists $\phi \in \refset_D(\mathcal{A}, \rho)$, $x\in \V$, $\beta\in \F(\L^+)$ and $m\in\N$ such that
$\gamma_\rho = \beta(x/\sk^m_\rho)$, 
$\free(\sk^m_\rho) = \free(\phi) \setminus (V \cup \fixed_D(\rho))$ with $\beta(x/\sk^m_\rho)$ admissible and
\begin{equation}
...
The intuition behind a \textit{constrained-proof} is that it is a proof in which certain variables are required to behave as constants and it does not require to place axioms in the reference system but apply rules directly to them.


Note that each \textit{constrained-proof} with $V=\emptyset$ can be transformed by inserting some references with axioms to the begining of each context where required in $D$ and renumerating the rest of references.
