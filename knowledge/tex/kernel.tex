...
r_1 \prec r_2
\end{equation}
iff there exists $s\in\N^*$ such that $r_2 = [*r_1, *s]$ and $s\not=[]$.
\begin{equation}
r_1 \preceq r_2
\end{equation}
...
\begin{proposition}
If $r_1$ and $r_2$ are roots of a context $D$, then $r_1 = r_2$. 
\end{proposition} 

We can then introduce a function $\root(D) = r$ where $r$ is a root of a context $D$.

$\overline{D} \defeq D \cup \root(D)$.

\begin{definition}
Let $D$ be a context.
\begin{equation}
\final(D) = [*\root(D), k_0]\in D,
\end{equation}
such that for all $k\in\N$ and $k > k_0$, $[*\root(D), k_0]\not\in D$.
\end{definition}
...
\begin{enumerate}
\item $D\subset \N^*$ and $D$ is a context,
\item $\gamma: \overline{D} \to \F^+$. 
\end{enumerate}
\end{definition}

\begin{definition}
A reference system $(D, \gamma)$ is a proof of $\psi\in\F^+$ with respect to $\mathcal{A}\subset F^+$
iff there exist functions $\alpha, \phi: \overline{D}\to \F^+$ such that for any $\rho\in \overline{D}$, $d = |\rho| - 1$:
\begin{enumerate}
\item $\gamma_\rho = \ulcorner \alpha_\rho \to \phi_\rho \urcorner$,
\item for any $c_{d\prime}$ in $\gamma_\rho$, $d\prime \leq d$,
\item for any $\sk_{\rho\prime}$ in $\gamma_\rho$, $\rho\prime \leq \rho$.
\item if $\{r\in D: \rho \prec r\} = \emptyset$, then $\alpha_\rho = \truth$ and one of the following holds:
\begin{enumerate}
\item (AXM) $\phi_\rho\in \mathcal{A}$,
\item (THR) there exists $\eta < \rho$ such that $\phi_\rho = \gamma_\eta$,
\item (ASM) there exists $\eta \prec \rho$ 
such that $\phi_\rho = \alpha_\eta$.  
\item (MOD) there are $\eta_1, \eta_2 < \rho$, such that
\begin{equation}
\begin{cases}
\gamma_{\eta_1} = \ulcorner \phi_{\eta_1} \to \phi_\rho \urcorner,\\ 
\gamma_{\eta_2} = \ulcorner \truth \to \phi_{\eta_1} \urcorner,
\end{cases}
\end{equation}
...
\begin{cases}
\alpha_\eta = \truth,\\
\phi_\eta = \phi_\rho(x/c_d),
\end{cases}
\end{equation}
\item (SKO) there exists $\eta < \rho$ and $x\in V$ and $\beta\in \F^+$ such that
$\phi_\rho = \beta(x/sk_\rho)$ and
\begin{equation}
\begin{cases}
\alpha_\eta = \truth,\\
\phi_\eta = \ulcorner \exists x. \beta\urcorner,
\end{cases}   
\end{equation}
...
\begin{cases}
\alpha_\eta = \truth,\\
\phi_\rho = \ulcorner \forall x. \phi_\eta \urcorner,
\end{cases}
\end{equation}
and for any $\zeta \prec \rho$, $x\not\in\free(\alpha_\zeta)$.
\end{enumerate}
\item if $D^\prime = \{r\in D: \rho \prec r\} \not= \emptyset$, for 
$\eta = \final(D^\prime)$, we have
\begin{equation}
\begin{cases}
\alpha_\eta = \truth,\\
\phi_\eta = \phi_\rho,
\end{cases}
\end{equation}
\item for $\eta = \root(D)$, $\gamma_\eta = \ulcorner \truth\to\phi \urcorner$.
\end{enumerate}
\end{definition}
