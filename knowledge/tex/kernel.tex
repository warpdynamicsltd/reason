...
The idea of managing local assumptions within proofs has a long history in proof theory, dating back to the Jaśkowski-Fitch style of natural deduction, where subproofs or “boxes” represent temporary contexts that are opened and discharged (see the survey in the Stanford Encyclopedia of Philosophy \cite{sepND}). 

Several systems have sought to formalize or mechanize this structure. Nested sequent calculi \cite{brunnler2010}, \cite{pimentel2019} generalize Gentzen's sequent systems by organizing sequents in a tree, thereby making the hierarchical proof structure explicit. 

Standefer \cite{standefer2014} investigates translations between tree-structured and linear natural-deduction proofs, clarifying how subproof hierarchies can be represented sequentially. 

Labelled deductive systems \cite{broda2004} extend natural deduction by attaching labels to formulas, capturing contextual or modal information within the syntax; this approach resembles the role of reference indices in our system. Structured proof languages such as Isar \cite{nipkow2002,wenzel2019} and teaching environments like ProofWeb \cite{kaliszyk2008} pursue the same goal from a computational perspective, aiming for human-readable, block-structured proofs with machine-checkable semantics.

Compared with these systems, context-proofs provide an explicit and uniform treatment of proof structure through finite-sequence references, a prefix relation defining contexts, and a reference-formula mapping that formalizes mathematical reasoning within nested subcontexts. While sharing conceptual roots with the frameworks above, our formalization offers a purely syntactic and general mechanism for hierarchical proof management that remains compatible with first-order logic and conducive to mechanized verification.
