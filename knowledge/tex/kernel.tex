...
\begin{definition}[\textbf{context consts}]
...
\begin{lemma}
Let $r\in \N^*$ and $\alpha, \beta, \gamma \in \F^+$ such that for any constant $c_d$ in them, we have $d \leq d(r)$ and for any constant $\sk_\eta$ in them, we have $\eta \leq r$.
...
\begin{lemma}[ALH]
\label{ALH}
Let $r\in \N^*$ and $\phi, \psi\in \F^+$ such that for any constant $c_d$ in them, we have $d \leq d(r)$ and for any constant $\sk_\eta$ in them, we have $\eta \leq r$.
...
[*r, 0, 4] %$\mapsto \forall x.\psi(x) $% # GEN $\gamma_{*r, 0, 4}$
\end{lstlisting}
