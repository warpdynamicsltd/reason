...
As it will play an important role in one inverence rule, we will justify the concept of \textit{admissible substitiution}.

Let's first recall the following definition.
\begin{definition}
Let $\L$ be a signature of a language.
Let $\M$ be a model of $\L$ with domain $A$ and let $j:\V\to A$ be an interpretation of variables. Then we define  
\end{definition}
...
Let $\L$ be a signature of a language. Let $\alpha\in\F(\L)$ and let $\tau$ be an arbitrary term. If $\alpha(x/\tau)$ is admissible, then for any model $\M$ of $\L$ with domain $A$ and any interpretation of variables $j:\V\to A$ we have
\begin{equation}
...
If $\alpha, \beta$ are isomorphic then for any model $\M$ of $\L$ with domain $A$ and any interpretation of variables $j:\V\to A$ we have
$$
...
$\gamma_\rho = \beta(x/\sk^m_\rho)$, $\free(\gamma_\eta) = \free(\sk^m_\rho)$ with $\beta(x/\sk^m_\rho)$ admissible and
\begin{equation}
...
[*r, 0, 1] %$\mapsto \big(\forall x. (\phi\to\psi(x))\big)\to(\phi\to\psi(x))$% # Axiom ALL
...
[*r, 0, 1] %$\mapsto \forall x. (\langle \psi \rangle_x (x) \to\phi)$% # ISO $\gamma_{*r, 0, 0}$
[*r, 0, 2] %$\mapsto \exists x.\psi(x)$% # ASM $\gamma_{*r, 0}$
[r*, 0, 3] %$\mapsto \exists x.\langle \psi \rangle_x (x)$% # ISO $\gamma_{*r, 0, 2}$
[*r, 0, 4] %$\mapsto\langle \psi \rangle_x(\sk^m_{[*r, 0, 2]})$% # SKO $\gamma_{*r, 0, 3}$
[*r, 0, 5] %$\mapsto \big(\forall x. (\langle \psi \rangle_x(x)\to\phi)\big)\to(\langle \psi \rangle_x(\sk^m_{[*r, 0, 2]})\to\phi)$%, # ALL
[*r, 0, 6] %$\mapsto \langle \psi \rangle_x(\sk_{[*r, 0, 2]})\to\phi$% # MOD $\gamma_{*r, 0, 5}, \gamma_{*r, 0, 1}$
[*r, 0, 7] %$\mapsto \phi$% # MOD $\gamma_{*r, 0, 6}, \gamma_{*r, 0, 4}$
\end{lstlisting}
Note that the assumption that $x\not\in \text{free}(\phi)$ is needed for 
$\ulcorner \langle \psi \rangle_x(x)\to\phi \urcorner(x/\sk^m_{[*r,0,2]})$ to become $\langle \psi \rangle_x(\sk^m_{[*r,0,2]})\to\phi$.
\end{proof}
...
\label{induction-a-2}
&\M_i \text{ is a model of } \overline{\L_i} \text{ for } 0 < i < k, \\
\label{induction-a-3}
...
\begin{enumerate}
\item (AXM) $\gamma_{r_k}\in T \cup \mathfrak{A}$. If $\gamma_{r_k}\in T$, then $(\M_{k-1}, j)\models\gamma_{r_k}$. If $\gamma_{r_k}\in \mathfrak{A}$, then $(\M_{k-1}, j)\models\gamma_{r_k}$ since $\gamma_{r_k}$ is a logical axiom. Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$. Then $(\M_k, j)\models\gamma_{r_k}$. Note that the above argument works for $k = 0$ without assumptions (\ref{induction-a-1}), (\ref{induction-a-2}) and (\ref{induction-a-3}). 

\item (ASM) $\gamma_{r_k} = \alpha$. Then trivially $(\M_k,j)\models \gamma_{r_k}$. Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$. Then
...
So $(\M_{k-1}, j)\models \ulcorner \gamma_{\eta_2} \to \gamma_{r_k} \urcorner, \gamma_{\eta_2}$ and therefore $(\M_{k-1},j)\models \gamma_{r_k}$. Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$. Then $(\M_k,j)\models \gamma_{r_k}$.
\item \label{gen} (GEN) there exists $\eta < r_k$ and $x\in \V\setminus V$ such that
$\gamma_{r_k} = \ulcorner \forall x. \gamma_\eta \urcorner$,
and $x\not\in\free(\alpha)$. This can only hold for $k > 0$.  Since $\gamma_\eta\in T_k$, 
...
Then we can repeat the reasoning from (\ref{assumption-alpha}) for an interpretation $j\cfrac{a}{x}$ up to this point and got $(\M_{k-1}, j\cfrac{a}{x})\models \gamma_\eta$.
This is possible because construction of $\M_{k-1}$ nowhere depends on $j$.
But since $a\in A$ was arbitrary chosen, we have $(\M_{k-1}, j)\models \forall x. \gamma_\eta $, and thus $(\M_{k-1},j)\models \gamma_{r_k}$. Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$. Then $(\M_k,j)\models \gamma_{r_k}$.
...
$\free(\sk^m_{r_k})=\free(\gamma_\eta)$ with $\beta(x/\sk^m_\rho)$ admissible. 
This can only hold for $k > 0$.
...
Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$ which models $\sk^m_{r_k}$ as defined above.
...
one can show that $(\context(D, r_k), \gamma)$ is a constrained-proof of $\gamma_{r_k}$ with respect to $\mathfrak{A}\cup T_k$ with fixed variables $\{\free(\alpha)\}$. Since $\M_{k-1}$ is a model of $\L_k$, $(\M_{k-1},j)\models T_k$ and $|\context(D, r_k)| < n$, by induction assumption (\ref{induction-1}), we have $(\M_{k-1}, j)\models\gamma_{r_k}$.
Let $\M_k$ be an arbitrary chosen but fixed extension of model $\M_{k-1}$ onto $\overline{\L_k}$. Then $(\M_k, j)\models\gamma_{r_k}$.
Note that the above argument works for $k = 0$ without any of without assumptions (\ref{induction-a-1}), (\ref{induction-a-2}) and (\ref{induction-a-3}). 
\end{enumerate}
...
$\phi = \ulcorner \alpha\to\omega \urcorner \in \F(\L_r)$. Since $\M_{k_f}$ is an extension of $\M$ but $\M$ is a model of $\L_r$, we have $(\M, j)\models\omega$ and then $(\M, j)\models\phi$.
\end{proof}
