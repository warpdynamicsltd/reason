...
\end{equation}
\end{enumerate}
\item if $\context(D, \rho) \not= \emptyset$, then there exists $\alpha\in\F(\L^+)$
 such that
\begin{equation}
...
\begin{definition}
\label{provability}
Let $\mathfrak{B}$ by all formulas from $\F(\L^+)$ which can be obtained by schemas from Lemma \ref{TRN}(TRN), Lemma \ref{ALH}(ALH) and Lemma \ref{EXH}(EXH). Let $\mathfrak{A}^s = \mathfrak{A} \cup \mathfrak{B}$.
\end{definition}
