...
\label{proof}
A proof step $p=(r^\prime, B, s, \phi)$ is a proof of a formula $\phi$ iff
...
If $\phi$ is a formula for which $x\not\in \text{free}(\phi)$ and $\psi$ is an arbitrary formula, then 
\begin{equation}
\ulcorner \big(\forall x. (\psi(x)\to\phi)\big)\to\big(\exists x.\psi(x)\to\phi\big) \urcorner \in \text{Cn}_R.
\end{equation} 
\end{fact}
\begin{proof}
\indent
