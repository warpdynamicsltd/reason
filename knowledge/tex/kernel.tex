...
\\
\\
(GEN) there exists $\eta < \rho$ and $x\in \V\setminus (V \cup \fixed_D(\rho))$ such that
\begin{equation}
\gamma_\rho = \ulcorner \forall x. \gamma_\eta \urcorner,
\end{equation}
\end{definition}
...
one can show that $(\context(D, r_k), \L^+, \gamma)$ is a constrained-proof of $\gamma_{r_k}$ with respect to $\mathfrak{A}\cup T_k$ with fixed variables $V \cup free(\alpha)$. Since $\M_{k-1}$ is a model of $\L_k$, $(\M_{k-1},j)\models T_k$ and $|\context(D, r_k)| < n$, by induction assumption (\ref{induction-1}), we have $(\M_{k-1}, j)\models\gamma_{r_k}$.
